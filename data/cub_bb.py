...
#DATASET_DIR = '/dccstor/moshel/datasets/CUB-2011-BB'
...
class Cub(data.Dataset):
    ...
    def __getitem__(self, index):
        img, label, bb = self.data[index], self.labels[index], self.bb[index]
...
class FewShotDataloader:
    ...
    def ids_to_tensors(self, examples):
        ...
        bbs = torch.stack([torch.Tensor(self.dataset[img_idx][-1]) for img_idx, _ in examples], dim=0)
        bbs = torch.cat((bbs[:,0:2], bbs[:,0:2]+bbs[:,2:4]),1) # [left, top, width, height] --> [left, top, right, bottom]
    ...
    def get_iterator(self, epoch=0):
        ...
        def load_function(iter_idx):
            ...
            Xt, Yt, Bt = self.ids_to_tensors(quer_ids)
            Xe, Ye, Be = self.ids_to_tensors(reps_ids)
            Bt = Bt.unsqueeze(dim=1)
            return Xe, Ye, Be, Xt, Yt, Bt, 1, 1
...
if __name__ == "__main__":
    ...
    ds = Cub(phase='test')
    dl = FewShotDataloader(ds)

    for i, batch in enumerate(dl(), 1):
        data_support, labels_support, data_query, labels_query, bb_query, _ = batch

        # img, label, bb = cls[57]
        img = data_query[0][0]
        bb = bb_query[0][0].numpy() # [left, top, width, hight]
