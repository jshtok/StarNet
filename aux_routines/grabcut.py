...
# class GrabCut:
#     def __init__(self, img, mask, gmm_components=5):
...
def heatmap2bbox_GC(img, mask,threshold= [],Niters = 1):
    # print documentation
    # Loading images
    # if len(sys.argv) == 2:
    ...
    # while(1):
        ...
        #         rect_or_mask = 1
    mask = pre_process_mask(mask, threshold=threshold)
    # mask = adaptive_threshold(mask)
    mask = mask.astype(np.uint8)
    bgdmodel = np.zeros((1, 65), np.float64)
    fgdmodel = np.zeros((1, 65), np.float64)

    # process the mask ---------------------------------------------------
    if False : # Use the local GrabCut impl.
        gc = GrabCut(img, mask)
        gc.run(skip_learn_GMMs=False)
        skip_learn_GMMs = False
    else:
        cv.grabCut(img, mask, None, bgdmodel, fgdmodel, Niters, cv.GC_INIT_WITH_MASK)


    mask2 = np.where((mask == 1) + (mask == 3), 255, 0).astype('uint8')
    output = cv.bitwise_and(img, img, mask=mask2)
        # cv.imwrite('/dccstor/alfassy/tmp/tmp.png', output)
        # cv.imwrite('/dccstor/alfassy/tmp/tmp2.png', img)
        # TODO: gen bbox from image
    return mask2


def pre_process_mask(mask, threshold=[50]):
    result = mask
    result = np.where(result < threshold[0], np.tile(DRAW_BG['val'], result.shape), result)
    condition_a = result >= threshold[0]
    # condition_b = result < (threshold[1]+1)
    # result = np.where(a * b, np.tile(DRAW_PR_FG['val'], (result.shape)), result)
    result = np.where(condition_a, np.tile(DRAW_PR_FG['val'], (result.shape)), result)
    # result = np.where(mask > threshold[1], np.tile(DRAW_FG['val'], (result.shape)), result)
    return result

def pre_process_mask2(heat_map, threshold=[], percentage=0.15):
    if not threshold:
        low_n_percentage = int(np.size(heat_map)*percentage)
        low_indices = np.ravel(heat_map).argsort()[:low_n_percentage]
        result_map = np.ones_like(heat_map)*DRAW_PR_FG['val']
        result_map = np.ravel(result_map)
        result_map[low_indices] = DRAW_BG['val']
        result_map = result_map.reshape(heat_map.shape)
    else:
        result_map = heat_map
        result_map = np.where(result_map < threshold[0], np.tile(DRAW_BG['val'], result_map.shape), result_map)
        condition_a = result_map >= threshold[0]
        result_map = np.where(condition_a, np.tile(DRAW_PR_FG['val'], (result_map.shape)), result_map)
        # condition_b = result < (threshold[1]+1)
        # result = np.where(a * b, np.tile(DRAW_PR_FG['val'], (result.shape)), result)
        # result = np.where(mask > threshold[1], np.tile(DRAW_FG['val'], (result.shape)), result)
    return result_map

# def adaptive_threshold(heat_map, std_factor=1, iterations=5):
#     valid = np.argwhere(heat_map != 0)
#     for iter in range(iterations):
#         mean = np.mean(heat_map[valid[:,0],valid[:,1]])
#         std = np.std(heat_map[valid[:,0],valid[:,1]])
#         valid = np.argwhere(np.abs(heat_map - mean) < (std_factor * std))
#     result_map = heat_map
#     # TODO: create final map
#     return valid